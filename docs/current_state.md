...
- 热修复仍待同一 Samsung 真机覆盖普通 UGC、PGC、DASH 独立音视频、短/长视频、
  清晰度与分P切换、全屏、后台、应用内小窗和系统 PiP。首要验收是画面恢复并持续
  输出；若仍黑屏，须立即读取 `VideoOutput` 与 Decoder 字段继续定位 Surface/Texture。
  真机通过前第八批不算结束，缓冲/解码设置组也从“实现完成、待验证”退回“兼容性
  回退、待重新实现”。
...
- 最高优先先在报告问题的 Samsung SM-S9180 Android 16 上覆盖安装批次 8 全黑热修复
  v2 审计 APK，保持超分禁用，验证普通 UGC、PGC 和 DASH 独立音视频能否出画面；打开
  播放信息核对 `VideoOutput` 与 Decoder。画面恢复后再测清晰度/分P、全屏、后台、应用
  内小窗和系统 PiP；仍黑屏时保存这两个字段及新的错误日志，不继续其他第八批矩阵。

1. 热修复通过后，再使用批次 8 超分辨率 v2 审计 APK 对照禁用/效率/画质，覆盖
