...
- `README.md` 已更新当前 ExoPlayer 迁移进度、应用内小窗行为、默认开关状态和
  上游同步说明；本批提交后推送到 `origin/codex/android-exoplayer`。
...
根据 `docs/android_exoplayer.md` 中记录的 2026-07-26 真机反馈，以下 ExoPlayer 场景
已经过当前测试设备验证：
...
- 缩小/恢复动画是否连续，以及恢复时是否无重新缓冲、跳转或播放器重建；
- 竖屏、横屏、方形视频的小窗比例；
- 当前 UGC 分P、PGC 剧集、互动视频和本地文件的恢复参数；
- 应用内小窗进入系统 PiP，以及 PiP 全屏恢复详情页、关闭 PiP 不恢复页面；
- 有线耳机、蓝牙、其他音频应用或通话造成的音频焦点变化；
- 锁屏通知、媒体按键、息屏/亮屏和前后台生命周期。
