...
class DynamicsController
  ...
  /// 本次扫描刚识别出的 UP。
  ///
  /// 官方常看状态与动态流是两个请求，状态可能还没跟上新动态，
  /// 清红点时跳过这些 UP，避免把刚出现的新更新立刻抹掉。
  final Set<int> _freshMids = <int>{};
  ...
  /// 因此升级键名强制丢弃旧状态、重新建立基线。
  static const _badgeCacheVersion = 'v4';
  ...
  /// 内容被看过后清除该作者的红点。
  ///
  /// 与点侧栏头像等价，只是触发点换成了「打开该 UP 的动态详情」——
  /// 在动态流里直接点开一条动态时，用户其实已经看过这次更新了。
  void _onContentRead(int mid) {
    if (_unreadUps.remove(mid) == null) return;
    _freshMids.remove(mid);
    unawaited(_saveUnreadUps());
    if (loadingState.value case Success(:final response)) {
      final upList = response.upList;
      if (upList != null) {
        for (final up in upList) {
          if (up.mid == mid) up.hasUpdate = false;
        }
      ...
      if (res case Success(:final response)) {
        ...
        _freshMids
          ...
          ..addAll(
            response.updatedUps
                .map((up) => up.mid)
                .where((mid) => !_unreadUps.containsKey(mid)),
          );
        ...
        // 观看历史是服务端数据，能识别「在官方 App / 网页看过视频」这类
        // 本地无从感知的情况，因此每轮都用它剔除已经看过的红点。
        await _pruneSeenUps();
        await Future.wait([
  ...
  /// 依据观看历史剔除「其实已经看过」的红点。
  ///
  /// 只读取最近若干页：翻到比所有未读红点都更早的观看记录就没有继续的必要，
  /// 因此绝大多数情况下只是一次请求。观看历史拉取失败不影响红点本身。
  Future<void> _pruneSeenUps() async {
    if (_unreadUps.isEmpty) return;
    try {
      final watched = <(int, int)>[];
      // 最旧的未读红点时间：观看记录比它还早就没有参考价值了。
      final oldestUnreadAt = _unreadUps.values
          .map((up) => up.latestUpdateAt ?? 0)
          .where((ts) => ts > 0)
          .fold<int>(0, (min, ts) => min == 0 || ts < min ? ts : min);
      ...
      for (var page = 0; page < 3; page++) {
        ...
        if (res case Success(:final response)) {
          ...
          for (final item in list) {
            final mid = item.authorMid;
            final watchedAt = item.viewAt;
            if (mid != null && watchedAt != null) watched.add((mid, watchedAt));
          }
          final last = list.last;
          if (oldestUnreadAt > 0 &&
              last.viewAt != null &&
              last.viewAt! < oldestUnreadAt) {
            break;
          }
          // 历史接口用「上一条的 oid + view_at」作为下一页游标。
      ...
      final viewedAtByMid = DynamicUpUpdateResult.foldWatchHistory(watched);
      if (viewedAtByMid.isEmpty) return;
      DynamicUpUpdateResult.pruneSeenByWatchHistory(_unreadUps, viewedAtByMid);
      // 已被观看历史清掉的 UP 不再算「刚识别」，否则会一直挡着官方状态那条路径。
      _freshMids.removeWhere((mid) => !_unreadUps.containsKey(mid));
    } catch (_) {
      // 观看历史不可用时保持现状，红点逻辑不因此中断。
    }
  }
  ...
  ///
  /// [officialFlags] 表示这次响应来自官方常看列表（portal），其中的 `has_update`
  /// 才是服务端维护的已读状态。翻页取回的关注列表没有该字段，不能据此清红点。
  void _applyUnreadUps(
    FollowUpModel data, {
    required bool includeMissing,
    bool officialFlags = false,
  }) {
    ...
    if (officialFlags && _unreadUps.isNotEmpty) {
      // 官方状态能覆盖「在官方 App / 网页里读过」这类本地无从感知的情况：
      // 只要该 UP 出现在官方列表且 `has_update` 不是 true，就说明服务端认为你没更新未读。
      final before = _unreadUps.length;
      DynamicUpUpdateResult.pruneSeenByOfficialFlags(
        _unreadUps,
        upList,
        freshMids: _freshMids,
      );
      if (_unreadUps.length != before) shouldSave = true;
    }
  ...
  bool customHandleResponse(bool isRefresh, Success<FollowUpModel> response) {
    ...
    _applyUnreadUps(
      ...
      // 刷新走的是 portal，才带官方的 has_update 状态。
      officialFlags: isRefresh,
    );
