...
class DynamicUpUpdateResult {
  ...
  /// 把观看历史折叠成「每个 UP 最近一次观看其视频的时间」。
  ///
  /// [entries] 是 (作者 mid, 观看时间秒) 的序列，允许同一 UP 出现多次。
  /// 观看历史是服务端数据，因此不论用户在本 App 还是官方 App / 网页看的视频，
  /// 都会体现在这里 —— 这是唯一能识别「在别处看过」的信号。
  static Map<int, int> foldWatchHistory(Iterable<(int, int)> entries) {
    final result = <int, int>{};
    for (final (mid, viewAt) in entries) {
      if (mid <= 0 || viewAt <= 0) continue;
      final old = result[mid];
      if (old == null || viewAt > old) result[mid] = viewAt;
    }
    return result;
  }
  ...
  /// 按观看历史剔除「其实已经看过」的红点。
  ///
  /// 判定规则：该 UP 最新一条动态是视频投稿，且发布时间之后用户看过他的视频，
  /// 就认为这次更新已看过。之所以是「该 UP 的视频」而不是精确到某一条，是因为官方
  /// 接口只提供 UP 级别的更新状态（`has_update`），视频与动态之间没有统一的已读标识可对齐。
  ///
  /// 两类条目不参与判断，避免误清：
  ///   * 没有发布时间 —— 来自官方常看列表的红点，无从与观看时间比较；
  ///   * 最新动态不是视频 —— 图文更新没有观看历史，看了他的旧视频不代表看过这条图文。
  static void pruneSeenByWatchHistory(
    Map<int, UpItem> unreadUps,
    Map<int, int> viewedAtByMid,
  ) {
    if (unreadUps.isEmpty || viewedAtByMid.isEmpty) return;
    unreadUps.removeWhere((mid, up) {
      if (up.isVideo != true) return false;
      final publishedAt = up.latestUpdateAt;
      if (publishedAt == null || publishedAt <= 0) return false;
      final viewedAt = viewedAtByMid[mid];
      return viewedAt != null && viewedAt >= publishedAt;
    });
  }
  ...
  static void pruneSeenByOfficialFlags(
    ...
    for (final up in officialUps) {
      if (up.hasUpdate == true || freshMids.contains(up.mid)) continue;
